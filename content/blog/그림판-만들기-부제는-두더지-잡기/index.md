...
이를 해결하기 위해서는 ( 작업 내용을 저장할 수 있고 && clearRect 대상이 아닌 ) 또 다른 캔버스를 하나 생성해야 한다. 
...
겹 캔버스를 양옆으로 펼쳐 놓으면 어떤 방식으로 그림이 그려지는지 좀 더 명확하게 이해될 것이다. (아래의 이미지 참고)
...
모양이 닫혀있는 직사각형과 달리 타원은 놀랍게도 닫히지 않았다!! 도형의 모양은 닫혀있는 게 맞는데... ellipse() 메소드는 좀 다르게 동작하는 것 같다. 따라서 beginPath()를 사용해 원이 그려질 때마다 새로운 패스를 생성해야 한다.\
\
...
아직은 이미지 확장자를 선택할 수 없지만(기본 jpg), png로 저장할 경우를 위해 되도록 지우개인 척을 하는 하얀색 펜의 사용을 지양해야한다. 
...
그런데.. 그린 그림을 저장하는 기능을 만들고 보니 저장된 그림이 어딘가 이상했다. 지우개로 지운 투명한 부분이 jpg로 저장하고 보니 검색으로 나타났던 것이다...
...
친구들이 직접 사용하면서 이상한 부분을 알려주었다. 아직.. 기능이 별로 없지만 덕분에 이것 저것 고칠 수 있었다. 
